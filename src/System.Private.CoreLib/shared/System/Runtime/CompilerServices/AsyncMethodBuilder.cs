...
namespace System.Runtime.CompilerServices
{
    ...
    internal static class AsyncMethodBuilderCore // debugger depends on this exact name
    {
        ...
        public static void SetResult<TResult>([AllowNull] ref Task<TResult> task, TResult result)
        {
            ...
            if (task == null)
            {
                task = AsyncTaskMethodBuilder<TResult>.GetTaskForResult(result);
                Debug.Assert(task != null, $"{nameof(AsyncTaskMethodBuilder<TResult>.GetTaskForResult)} should never return null");
            }
            else
            {
